...
class FileSearch {
	static VirtualFile findFileByName(String filePath, @NotNull Project project, boolean searchInLibraries = false) {
		...
		if (files.size() > 1) {
			def filePaths = files.collect { it.canonicalPath }.join("\n")
	...
	static Set<VirtualFile> findAllFilesByName(String filePath, @NotNull Project project, boolean searchInLibraries = false) {
